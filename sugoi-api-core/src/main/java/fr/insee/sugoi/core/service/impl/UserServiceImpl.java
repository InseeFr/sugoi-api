...
public class UserServiceImpl implements UserService {
    ...
    try {
      ...
      if (findById(realm, storage, user.getUsername()).isEmpty()) {
        ...
        return findById(realm, storage, userName)
            .orElseThrow(() -> new UserNotCreatedException("Cannot create user " + userName + " in realm " + realm));
      }
      throw new UserAlreadyExistException("User " + user.getUsername() + " already exist in realm " + realm);
    } catch (Exception e) {
      sugoiEventPublisher.publishCustomEvent(realm, storage, SugoiEventTypeEnum.CREATE_USER_ERROR,
          Map.ofEntries(Map.entry(EventKeysConfig.USER, user), Map.entry(EventKeysConfig.ERROR, e.toString())));
  ...
  @Override
  public ProviderResponse update(String realm, String storage, User user) {
    try {
      ProviderResponse response = storeProvider.getWriterStore(realm, storage).updateUser(user);
      sugoiEventPublisher.publishCustomEvent(realm, storage, SugoiEventTypeEnum.UPDATE_USER,
          Map.ofEntries(Map.entry(EventKeysConfig.USER, user)));
      return response;
    } catch (Exception e) {
      sugoiEventPublisher.publishCustomEvent(realm, storage, SugoiEventTypeEnum.UPDATE_USER_ERROR,
          Map.ofEntries(Map.entry(EventKeysConfig.USER, user), Map.entry(EventKeysConfig.ERROR, e.toString())));
      throw e;
    }
  ...
  public void delete(String realmName, String storage, String id) {
    try {
      findById(realmName, storage, id)
          .orElseThrow(() -> new UserNotFoundException("Cannot find user " + id + " in realm " + realmName));
      ...
      sugoiEventPublisher.publishCustomEvent(realmName, storage, SugoiEventTypeEnum.DELETE_USER_ERROR,
          Map.ofEntries(Map.entry(EventKeysConfig.USER_ID, id), Map.entry(EventKeysConfig.ERROR, e.toString())));
  ...
  public Optional<User> findById(String realmName, String storage, String id) {
    ...
    try {
      ...
      sugoiEventPublisher.publishCustomEvent(realmName, storage, SugoiEventTypeEnum.FIND_USER_BY_ID_ERROR,
          Map.ofEntries(Map.entry(EventKeysConfig.USER_ID, id), Map.entry(EventKeysConfig.ERROR, e.toString())));
      return Optional.ofNullable(user);
    }
  }
    ...
    try {
      if (storage != null) {
        result = storeProvider.getReaderStore(realm, storage).searchUsers(userProperties, pageable,
            typeRecherche.name());
        result.getResults().forEach(user -> {
        ...
        for (UserStorage us : r.getUserStorages()) {
          ReaderStore readerStore = storeProvider.getStoreForUserStorage(realm, us.getName()).getReader();
          PageResult<User> temResult = readerStore.searchUsers(userProperties, pageable, typeRecherche.name());
          temResult.getResults().forEach(user -> {
            user.addMetadatas(EventKeysConfig.REALM, realm);
          ...
          if (result.getTotalElements() >= result.getPageSize()) {
            sugoiEventPublisher.publishCustomEvent(realm, storage, SugoiEventTypeEnum.FIND_USERS,
                Map.ofEntries(Map.entry(EventKeysConfig.USER_PROPERTIES, userProperties),
      ...
      sugoiEventPublisher.publishCustomEvent(realm, storage, SugoiEventTypeEnum.FIND_USERS_ERROR,
          Map.ofEntries(Map.entry(EventKeysConfig.USER_PROPERTIES, userProperties),
              Map.entry(EventKeysConfig.PAGEABLE, pageable), Map.entry(EventKeysConfig.TYPE_RECHERCHE, typeRecherche),
