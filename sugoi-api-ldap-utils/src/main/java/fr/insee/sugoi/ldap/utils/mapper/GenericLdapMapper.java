        ...
        try {
          ...
          if (field.getDeclaredAnnotationsByType(AttributeLdapName.class).length > 0) {
            ...
            if (field.getDeclaredAnnotationsByType(MapToAttribute.class).length > 0) {
              Field userField = object.getClass().getDeclaredField(field.getAnnotation(MapToAttribute.class).value());
              userField.setAccessible(true);
              userField.set(object, result.getAttributeValue(field.getAnnotation(AttributeLdapName.class).value()));
            }

            if (field.getDeclaredAnnotationsByType(MapToMapElement.class).length > 0) {
              Field userField = object.getClass().getDeclaredField(field.getAnnotation(MapToMapElement.class).name());
              userField.setAccessible(true);
