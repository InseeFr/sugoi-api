...
public class LdapReaderStore extends LdapStore implements ReaderStore {
      ...
      try {
        ...
        List<User> filteredUsers =
            results.getResults().stream()
                .filter(
                    u ->
                        stripAccents((String) u.getAttributes().get("common_name"))
   ...
   */
  private <M extends SugoiObject> Filter getFilterFromObject(
      M object, LdapMapper<M> mapper, String searchType, boolean encodeCommonName) {
    Assert.isTrue(
    ...
    List<Filter> attributeListFilter = getAttributesFilters(attributes, encodeCommonName);
  ...
  private List<Filter> getAttributesFilters(List<Attribute> attributes, boolean encodeCommonName) {
    List<Filter> filters =
        attributes.stream()
            .filter(
                attribute ->
